...
const MUT = [
  ...
  { n:'صورةُ الشاخص لم تعد إلزاميةً على الميدان', g:'scripts/audit-rules.mjs',
    a:"if (!office && !NEWSITE.photos[0]){ toast(t('صورة الشاخص إلزامية للاعتماد')); return; }", b:'' },
  /* V17.57–V17.59: ما يُضاف من المكتب ليس زيارة، والمساحةُ مخيمٌ لا شبكة */
  { n:'نقطةُ المكتب تُحتسَب زيارة', g:'scripts/audit-draw.mjs',
    a:"  if (!office) nsVisitRec(id);", b:"  nsVisitRec(id);" },
  { n:'مساحةُ المخيم تُملأ نقاطًا', g:'scripts/audit-draw.mjs',
    a:"  if (ROUTE.mode === 'area' && ROUTE.type === 'مخيم'){ areaCampSave(); return; }", b:'' },
  { n:'إجماليُّ ورقةِ نظرةٍ عامة لا يساوي عمودَه', g:'scripts/audit-exports.mjs',
    a:"out.push(['الإجمالي','', K.total.n, K.total.sv, K.total.ins]);",
    b:"out.push(['الإجمالي','', K.total.n + 7, K.total.sv, K.total.ins]);" },
  ...
  { n:'شاشةٌ تعود إلى بيانةٍ مزروعةٍ لها مصدرٌ حيّ', g:'scripts/audit-data.mjs',
    a:"      + cardFlush(t('موظفو') + ' ' + esc(crew.n) + ' — ' + nm(techs.length),",
    b:"      + cardFlush(t('موظفو') + ' ' + esc(crew.n) + ' — ' + nm(DATA.techs.length)," },
