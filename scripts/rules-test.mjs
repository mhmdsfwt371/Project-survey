...
/* الحذفُ صار للمهندس والمدير على من دونهم (V15.96) — فما يُختبَر هنا أن
   المهندسَ لا يحذف نفسَه؛ وحذفُ الفنيِّ يُختبَر في آخر الملف بوثيقةٍ لا يحتاجها
   ما بعدها (كان الفحصُ القديمُ يحذفه فعلًا فتسقط ثمانيةُ فحوصٍ بعده). */
await deny('لا يحذف المهندسُ نفسَه',              deleteDoc(doc(as('eng'), 'users/eng')));
