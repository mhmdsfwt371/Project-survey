...
pend = 3; w.IDB_BAD = true; w.STATE.queue.push({ kind:'recs', id:'Q0', v:{ id:'Q0' }, at:Date.now() - 9000 }); w.render(1); await wait(60);
...
w.IDB_BAD = false; w.idbTellBad(true, 'UnknownError: Connection to Indexed Database server lost'); w.STATE.meta.online = false; w.render(1); await wait(40);
...
console.log('\n══ ٦ · حثُّ التثبيت على آيفون (V19.6) ══');
w.STATE.meta.online = true; w.STATE.queue.length = 0; w.idbBannerUpdate();
T(!d.getElementById('idbBanner'), 'الحفظُ معطَّلٌ والشبكةُ حاضرةٌ ولا شيءَ ينتظر: لا لافتة');
w.pwaIOS = () => true; w.pwaStandalone = () => false; w.localStorage.removeItem('nsk14.iosNudge'); w.render(1); await wait(40);
T(!!d.getElementById('iosNudge') && /إضافة إلى الشاشة الرئيسية/.test(d.getElementById('iosNudge').textContent) && /واتساب/.test(d.getElementById('iosNudge').textContent), 'آيفونُ غيرُ مثبَّت: تظهر خطواتُ التثبيت بعد الدخول (ومعها واتساب)');
d.querySelector('[data-iosnudge="done"]').dispatchEvent(new w.MouseEvent('click', { bubbles:true })); await wait(40);
