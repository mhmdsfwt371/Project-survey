...
/* ═══ ١٤ · ما فشل رفعُه يُعاد بمهلةٍ متزايدة (V17.40) ═══ */
T(/it\.tries < 12/.test(js) && /\[60000, 240000, 900000, 3600000, 10800000\]/.test(js),
  'وما فشل رفعُه لعطلٍ عابرٍ يُعاد: دقيقةٌ ثم أربعٌ ثم خمسَ عشرةَ ثم ساعةٌ ثم ثلاث');
T(/permission\|denied\|PERMISSION/.test(js) && /if \(hard \|\| it\.tries >= 12\)/.test(js),
  'ولا يُعزَل إلا ما لا يُصلحه التكرارُ — رفضُ القواعد أو حجمٌ فوق السقف');
T(/!it\.next \|\| it\.next <= nowQ/.test(js), 'والمؤجَّلُ لا يعيق ما بعده في الطابور');
